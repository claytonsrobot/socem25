...
import time
import xlsxwriter
import EI_Interaction_Fx_errorManagement as EI_Interaction_Fx
import EI_No_Interaction_Fx_errorManagement as EI_No_Interaction_Fx
...
def datafeed():
    ...
    if visualizeDatastream == True:# data displayed in scrollbars (default)
        ...
        '''
        RecordForce.time_label = Label(frame, text = "Time (sec)",font = ("arial", 14, "bold"), fg = "dodgerblue3", bg = "ghost white")
...
def run(self, ser):
    ...
    if vis == 's':# data displayed in scrollbars (default)
        ...
        RecordForce.Timelist = Listbox(self, yscrollcommand = scroll.set, bg = "ghost white",highlightbackground = "gray2", width = 7, height = 1, font = ("arial", 14, "bold"), fg = "dodgerblue2")
        ...
        RecordForce.Dislist = Listbox(self, yscrollcommand = scroll.set, bg = "ghost white",highlightbackground = "gray2", width = 7, height = 1, font = ("arial", 14, "bold"), fg = "dodgerblue2")
        ...
        RecordForce.Forcelist = Listbox(self, yscrollcommand = scroll.set, bg = "ghost white",highlightbackground = "gray2", width = 7, height = 11, font = ("arial", 14, "bold"), fg = "dodgerblue2")
    ...
    while RecordForce.collect == True: # GUI in fSerConnect()rontend controls value of collect to start/stop loop            
...
class FinalInputs(tk.Frame):
    ...
    def __init__(self, parent, controller): # automatically runs
        ...
        ''' GUI design, non-frame '''
        pageButtons = repeatPageButtons.showButtons(self, parent, controller)
        unit_label = Label(self, text=str("Mass unit is grams, diameter unit is millimeters."), font = ("arial", 12, "italic"), fg = "red4", bg="ghost white")        
    ...
    def calculateEI(peak_force, stemheight, barbottom, stemcount):
        ...
        EI_nocontact = EI_No_Interaction_Fx.EI_NoInteraction(peak_force, stemheight, barbottom, stemspacing_average) # the x value of the click does nothing other than find the nearest height from horz. It is not factored in to the number of beams or the character of the beams. 
...
#suppress internal EI calculation, reference external modules       
...
##    def otherBeams(i,h,l,s): # called after determining # of beams
##        EI_Interaction_Fx_betaA.insert(i + 1, np.arctan((EI_Interaction_Fx_gamma_length[i] * np.sin(EI_Interaction_Fx_betaA[i])) / EI_Interaction_Fx_dist_horz[i]))
##        phiV = (EI_Interaction_Fx_betaA[i])  # initially assume phi = previous beta
##        #n = (1 / np.tan(phiV))  # determine n
##        if phiV != 0:
##            n = (np.cos(phiV)/np.sin(phiV))  # determine n
##        else:
##            n = 0
...
##        EI_Interaction_Fx_q_len.insert(i + 1, b + math.sqrt(EI_Interaction_Fx_dist_horz[i] ** 2 + (EI_Interaction_Fx_gamma_length[i] * np.sin(EI_Interaction_Fx_betaA[i])) ** 2))  # effective cantilever beam length (base to applied force from previous beam or force bar)
##        EI_Interaction_Fx_gamma_length.append(gamma * l)  # store beam's gamma*length  
##        EI_Interaction_Fx_x_deflection.insert(i + 1, EI_Interaction_Fx_gamma_length[i + 1] * np.cos(EI_Interaction_Fx_betaA[i + 1]))  # x deflection at end of beam
##        #y.insert(i+1, EI_Interaction_Fx_gamma_length[i+1] * np.sin(beta[i + 1])) # y deflection at end of beam (not needed for EI calculation)
##        EI_Interaction_Fx_dist_horz.insert(i + 1, EI_Interaction_Fx_x_deflection[i + 1] - s)# stores horizontal distance beam extends past the next
##
##
...
##            # betaV needs edge case management - the solution should have been programmed already on the laptop, or the desktop
##            inside = (h - b) / (gamma * l)
...
##        EI_Interaction_Fx_x_deflection.insert(0, EI_Interaction_Fx_gamma_length[0] * np.cos(EI_Interaction_Fx_betaA[0]))  # stores x deflection
##        EI_Interaction_Fx_dist_horz.insert(0, EI_Interaction_Fx_x_deflection[0] - s)  # stores horizontal distance beam extends past the next
##        phiCorrection = EI_Interaction_Fx.MultiPhiCor(h, l, s, phiV) # correct phi (if h/l < 0.7)
##        EI_Interaction_Fx_phi.insert(0, phiCorrection)  # force vector angle w/ respect to undeflected axis
##
##        # Determine # of other beams:
##        if definite_beam_num == False:  # full interaction @ 1st beam's max deflection
##            i = 0
##            print("EI_Interaction_Fx_dist_horz=",EI_Interaction_Fx_dist_horz)
##            while EI_Interaction_Fx_dist_horz[i] > 0:  # will previous beam hit next beam? If so, run that beam through otherBeams()
##                print("EI_Interaction_Fx_dist_horz=",EI_Interaction_Fx_dist_horz)
##                print("i=",i)
##                EI_Interaction_Fx.otherBeams(i,h,l,s)
##                i += 1
##        else:  # definite number of beams in a row (may not be full/max interaction possible)
##            i = 0
##            while EI_Interaction_Fx_dist_horz[i] > 0 and i < beams-1:  # will previous beam hit next beam & does that beam exist? If so, run that beam through otherBeams()
##                EI_Interaction_Fx.otherBeams(i,h,l,s)
##                i += 1
...
##            den = ((EI_Interaction_Fx_x_deflection[-2] - s) * np.cos(EI_Interaction_Fx_phi[-1]) + (EI_Interaction_Fx_gamma_length[-2] * np.sin(EI_Interaction_Fx_betaA[-2]) * np.sin(EI_Interaction_Fx_phi[-1])))  # denominator
...
##                #num2 = EI_Interaction_Fx_forces[j + 1] * (EI_Interaction_Fx_gamma_length[j] * np.sin(EI_Interaction_Fx_betaA[j]) * np.sin(EI_Interaction_Fx_phi[j + 1]) + x[j] * np.cos(EI_Interaction_Fx_phi[j + 1]))  # force due to previous beam
##                num2 = EI_Interaction_Fx_forces[j + 1] * (EI_Interaction_Fx_gamma_length[j] * np.sin(EI_Interaction_Fx_betaA[j]) * np.sin(EI_Interaction_Fx_phi[j + 1]) + EI_Interaction_Fx_x_deflection[j] * np.cos(EI_Interaction_Fx_phi[j + 1]))  # force due to previous beam
##                den1 = (EI_Interaction_Fx_x_deflection[j - 1] - s) * np.cos(EI_Interaction_Fx_phi[j])  # denominator term 1
##                den2 = EI_Interaction_Fx_gamma_length[j - 1] * np.sin(EI_Interaction_Fx_betaA[j - 1]) * np.sin(EI_Interaction_Fx_phi[j])  # denominator term 2
...
##        knum1 = fx * (EI_Interaction_Fx_x_deflection[0] * np.cos(EI_Interaction_Fx_phi[0]) + (h - b1) * np.sin(EI_Interaction_Fx_phi[0]))  # numerator
...
##            kden2 = EI_Interaction_Fx_forces[1] * ((h - b1) * np.sin(EI_Interaction_Fx_phi[1]) + EI_Interaction_Fx_x_deflection[0] * np.cos(EI_Interaction_Fx_phi[1])) # denominator 2nd term due to interactions
...
##
##        ## COMPUTE EI ##
##        EI = (l * K) / (EI_Interaction_Fx_KO[0] * gamma1)
##
##        #t_num = len(EI_Interaction_Fx_betaA)  # total number of interacting beams @ 1st beam's deflection
...
##    definite_beam_num = True # CB 10/10/22
##    beams = 8 # num. of beams in a row (only used if "definite_beam_num" set to True)
##
##    # Model lists/arrays - each index is a beam's attribute (0 index = 1st beam, last index = last beam)
...
##    global EI_No_Interaction_Fx_dens
##
...
##    EI_No_Interaction_Fx_q_len = list()# effective l term in following k's equation 
##    EI_No_Interaction_Fx_KO = list() # stiffness coefficient
##    EI_No_Interaction_Fx_gamma_length = list() # gamma*l (longer rigid link length)
##    EI_No_Interaction_Fx_dist_horzens = list() # denominator terms for EI
##    EI_No_Interaction_Fx_dens = list()
##    
##    def clearAll(): # clears variables for new simulation 
...
##        EI_No_Interaction_Fx_dens.clear()
##
##    def Parametric_angle_coefficient(n): # returns c (parametric angle coefficient) when given n
##        if -4 < n <= -1.5:
##            c = 1.238945 + 0.012035*n + 0.00454*(n**2)
...
##    
##    def otherBeams(i,h,l,s): # called after determining # of beams
##        EI_No_Interaction_Fx_betaA.insert(i+1, np.arctan((EI_No_Interaction_Fx_gamma_length[0] * np.sin(EI_No_Interaction_Fx_betaA[0])) / (EI_No_Interaction_Fx_dist_horz[i])))
##        #phiV = EI_No_Interaction_Fx.beta[i] # initialy assum phi = previous beta
##        phiV = EI_No_Interaction_Fx_betaA[i] # initialy assum phi = previous beta
##        n = (1/np.tan(phiV)) # determine n
...
##        b = ((1 - gamma) * l) # update base length
##        EI_No_Interaction_Fx_x_deflection.insert(i+1, EI_No_Interaction_Fx_dist_horz[i]) # x distance from base of beam to force bar's positon @ 1st beam's max deflection
##        EI_No_Interaction_Fx_dist_horz.insert(i+1, EI_No_Interaction_Fx_x_deflection[i+1] - s) # horizontal distance beam extends past the next
##        EI_No_Interaction_Fx_q_len.insert(i+1, b + math.sqrt(EI_No_Interaction_Fx_x_deflection[i]**2 + (EI_No_Interaction_Fx_gamma_length[0]*np.sin(EI_No_Interaction_Fx_betaA[0]))**2)) # effective cantilever beam length (base to applied force)
##
##        return i
##
##    # MAIN FUNCTION THAT RETURNS THE SYSTEM'S MEAN EI
...
##            print("np.isnan(EI_No_Interaction_Fx_gamma_length[0]) =",np.isnan(EI_No_Interaction_Fx_gamma_length[0]))
...
##        EI_No_Interaction_Fx_x_deflection.insert(0, EI_No_Interaction_Fx_gamma_length[0] * np.cos(EI_No_Interaction_Fx_betaA[0]))  # stores x deflection
##
##        EI_No_Interaction_Fx_dist_horz.insert(0, EI_No_Interaction_Fx_x_deflection[0] - s)  # stores horizontal distance beam extends past the next
...
##
##        # Determine # of other beams:
##        numBeams = int(round(EI_No_Interaction_Fx_x_deflection[0]/s)) # number of additional beams hitting force bar at 1st one's max deflection
##        
...
##                EI_No_Interaction_Fx.otherBeams(i,h,l,s)
##                #EI_No_Interaction_Fx.EI_NoInteraction.otherBeams(i,h,l,s)j
##                i += 1
...
##                EI_No_Interaction_Fx.otherBeams(i,h,l,s)
##                #EI_No_Interaction_Fx.EI_NoInteraction.otherBeams(i,h,l,s)
##                i += 1
##                
##        # Calculations required to compute EI
##        for i in range(len(EI_No_Interaction_Fx_betaA)):
##            EI_No_Interaction_Fx_theta.insert(i, ((math.pi/2)-EI_No_Interaction_Fx_betaA[i])) # PRBM Theta
##            #s.insert(i, deltaTheta[i]/(gamma*(q[i]**2)))                     
##            EI_No_Interaction_Fx_dens.insert(i, (EI_No_Interaction_Fx_KO[i]*EI_No_Interaction_Fx_theta[i])/(EI_No_Interaction_Fx_q_len[i]**2)) # denominator terms for EI
##        EIden = sum(EI_No_Interaction_Fx_dens) # denominator sum term for EI
##	print("EIden=",EIden)
##        # Compute EI

'''
Title: EI_Interaction_Fx.py
Ex: EI=EI_Interaction(f,fb_h,stem_h,spacing)
Calculate highest avg stem EI, no interaction
OG Author: Austin Bebee
Contributor: Clayton Bennett
Last updated by Austin: 7/6/2020
Last updated by Clayton: 10/16/2022


Description:
Calculates lowest possible avg stem EI, full interaction
...
def EI_Interaction(f, h, l, s):
    ...
    if np.isnan(gl[0]):
        ...
        gl[0]= .852144*l # fix this to a more realistic value  
    ...
    if definite_beam_num == False:  # full interaction @ 1st beam's max deflection
        i = 0
        while d[i] > 0 and i<1000:  # will previous beam hit next beam? If so, run that beam through otherBeams()
            otherBeams(i)
            i += 1

...
class DataFeed(PassIn):
    ...
    def passData(self):
        ...
        if Config.visualizeDatastream == True:
            try: # puts data on GUI display by default (user can turn off)
                ...
                RecordForce.Forcelist.insert(END, str('%.2f' % self.gui_main_object.forcePushed[i]))
                RecordForce.Forcelist.see(END)
                RecordForce.Timelist.insert(END, str('%.2f' % self.gui_main_object.timeElapsed[i]))
            ...
            '''
                ...
                self.gui_main_object.errors.append('data append') # label 
                eCode = 'e4'
                self.gui_main_object.errorCodes.append(eCode)
        ...
        while RecordForce.hasStarted==True and RecordForce.hasSentStop==False:
                    ...
                    if RecordForce.hasSentStop == False:
                        try:
                            ...
                            if timetick > lasttimetick:
                                self.gui_main_object.timeElapsed.append(timetick)# list of self.gui_main_object.distanceTraveled time
                            else:
                                timetick = lasttimetick # good enough.
                                self.gui_main_object.timeElapsed.append(timetick)# list of self.gui_main_object.distanceTraveled time
                            self.gui_main_object.distanceTraveled.append(distance)# list of inches traveled @ does this happen with the whole list, or one element at a time?
                            self.gui_main_object.forcePushed.append(force)# list of force traveled
            ...
            # But also, it has primarily been entered if the serial connection has already timed out
                ...
                if nothingToRead>5: # if the while loop goes through five iterations, without seeing anything worth recording, give up.
                    ...
                    print("Hung up.")
                    SC.serial_reconnect()
                    self.gui_main_object.show_frame(self.gui_initial_inputs_object)
            else:
                hang +=1
    ...
    def runDataCollect(self):
        try:        
            RecordForce.sendStart()
        except:
            print("run fail")
            self.gui_main_object.errors.append('serial com. (start data)') # label 
            eCode = 'e2' # eCode = e2
            self.gui_main_object.errorCodes.append(eCode)
            print("eCode = "+eCode)
